...
// Release selection
const RELEASE_KEY = `overture_release_${location.origin}`;
let RELEASE = localStorage.getItem(RELEASE_KEY) || null;

function cacheKeyForRelease(release) {
  return `overture_files_${release}_${location.origin}`;
}
let CACHE_KEY = RELEASE ? cacheKeyForRelease(RELEASE) : null;
...
async function initReleasesUI() {
  ...
  if (!RELEASE) RELEASE = releases[0] || null;
  ...
  select.onchange = () => {
    ...
    CACHE_KEY = cacheKeyForRelease(RELEASE);
    fileCache = JSON.parse(localStorage.getItem(CACHE_KEY) || '{}');
    placesBbox = null;
    buildingsBbox = null;
    log(`Release set to ${RELEASE}`, 'success');
  };

  // Initialize cache key + cache state
  localStorage.setItem(RELEASE_KEY, RELEASE);
  CACHE_KEY = cacheKeyForRelease(RELEASE);
  fileCache = JSON.parse(localStorage.getItem(CACHE_KEY) || '{}');
...
async function getSmartFilteredFiles(dataType, bbox) {
  ...
  const baseUrl = `${PROXY}/files/${dataType}?release=${encodeURIComponent(RELEASE)}&xmin=${bbox.xmin}&xmax=${bbox.xmax}&ymin=${bbox.ymin}&ymax=${bbox.ymax}`;

  // If worker is building the index, it will return 202. Poll.
  let attempt = 0;
  while (true) {
    ...
    if (response.status === 202) {
      attempt++;
      log(`Building ${dataType} index for ${RELEASE} (first run)… [${attempt}]`);
      await sleep(2500);
      continue;
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(`files endpoint failed (${response.status}): ${txt}`);
    }
...
async function init() {
  try {
    ...
    log(`Ready (release ${RELEASE})`, 'success');
  } catch (e) {
